...
import type { RegistrationInfo } from '~/types/Type';

const form = ref<RegistrationInfo>({
  name: '',
  email: '',
  password: '',
...
const handleRegister = async () => {
  const { error } = await auth.register(form.value, 'admin');
...
<template>
  ...
  <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="handleRegister">
    <div class="mb-4">
      ...
      <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight" id="name" type="text" placeholder="xxxxx" v-model="form.name">
    </div>
    <div class="mb-4">
